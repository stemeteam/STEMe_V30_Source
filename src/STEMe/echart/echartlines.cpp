#include "echartlines.h"

QT_CHARTS_USE_NAMESPACE

EChartLines::EChartLines(QGraphicsItem *parent, Qt::WindowFlags wFlags)
...
EChartLines::~EChartLines()
{

}

void EChartLines::setSplines(QLineSeries *line, int i)
{
    ...
    if(m_chartGesture)
        {
      //  connect(m_spline[i-1], &QLineSeries::clicked, m_chartGesture, &EChartGesture::keepCallout);
        connect(m_spline[i-1], &QLineSeries::hovered, m_chartGesture, &EChartGesture::tooltip);
        }
}

QPointF EChartLines::getPositionMouse()
{
    QPointF p(0,0);
...
QPointF EChartLines::getValueMouse(int line)
{
...
void EChartLines::setLinesIndexNext()
{
m_linesIndex++;
...
void EChartLines::setPrecisionXY(int x, int y)
{
    if(m_chartGesture)m_chartGesture->setPrecisionXY(x,y);
}

void EChartLines::setTrackLine(EChartGesture::ETrackLine track)
{
    if(m_chartGesture)m_chartGesture->setTrackLine(track);
}

void EChartLines::init()
{
    this->chartInit(m_chartParameter,m_widget);
}


void EChartLines::init(Parameter parameter,int lines, QWidget *widget)
{
    m_chartParameter = parameter;
    m_spline.clear();
    m_lines = lines;
    for(int i=0;i<lines;i++){
        m_spline.append(new QLineSeries());
    }
    m_splineHelp = new QLineSeries();
    m_widget = widget;
    this->chartInit(m_chartParameter,m_widget);
}

void EChartLines::init(EChartLines::Parameter parameter, QLineSeries *lines, QWidget *widget)
{
    m_chartParameter = parameter;
    m_spline.clear();
    m_lines = 1;
...
void EChartLines::init(EChartLines::Parameter parameter, QList<QLineSeries*> lines, QWidget *widget)
{
    m_chartParameter = parameter;
    m_spline.clear();
    m_lines = lines.count();
...
void EChartLines::chartInit(Parameter parameter,QWidget *widget)
{
    ...
    m_chartGesture->show();
    for(int i=0;i<m_spline.count();i++){
      //  connect(m_spline[i], &QLineSeries::clicked, m_chartGesture, &EChartGesture::keepCallout);
      connect(m_spline[i], &QLineSeries::hovered, m_chartGesture, &EChartGesture::tooltip);
    }
}

void EChartLines::graph(double val)
{
    this->graph(val,m_linesIndex);
}

void EChartLines::graph(double val, int lines)
{
    this->graph(val,lines,m_scanType);
}

void EChartLines::graph(double val, int lines, ScanType scan)
{
...
void EChartLines::graph(QPointF val)
{
    this->graph(val,m_linesIndex);
}

void EChartLines::graph(QPointF val, int lines)
{
    this->graph(val,lines,m_scanType);
}

void EChartLines::graph(QPointF val, int lines, EChartLines::ScanType scan)
{
    if(lines==0||lines>m_spline.count())return;
    m_scanType = scan;
    //Calculator
...
void EChartLines::graph(QVector<QPointF> val)
{
    this->graph(val,m_linesIndex);
}

void EChartLines::graph(QVector<QPointF> val, int lines)
{
    this->graph(val,lines,m_scanType);
}

void EChartLines::graph(QVector<QPointF> val, int lines, EChartLines::ScanType scan)
{
...
void EChartLines::clear()
{
    this->chartClear();
}

void EChartLines::chartClear()
{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
void EChartLines::graphScroll(double val,int lines)
{
...
void EChartLines::graphCompact(double val, int lines)
{
    if(m_spline[lines-1]->count()>chartAxisX->max())chartAxisX->setMax(m_spline[lines-1]->count());
    m_spline[lines-1]->append(m_spline[lines-1]->count(),val);
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////
void EChartLines::graphScroll(QPointF val, int lines)
{
...
void EChartLines::graphCompact(QPointF val, int lines)
{
    m_spline[lines-1]->append(val.x(),val.y());
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
void EChartLines::graphScroll(QVector<QPointF> val, int lines)
{
...
void EChartLines::graphCompact(QVector<QPointF> val, int lines)
{
    for(int i=0;i<val.count();i++){
        m_spline[lines-1]->append(val.at(i).x(),val.at(i).y());
    }
...
void EChartLines::updateLineHelp(int x)
{
...
////////////////////////////////////////////////////////////////////Event for gesture
//![1]
bool EChartLines::sceneEvent(QEvent *event)
{
...
bool EChartLines::gestureEvent(QGestureEvent *event)
{
