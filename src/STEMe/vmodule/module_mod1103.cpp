...
void Module_MOD1103::on_pushButton_SetADD_clicked()
{
    ...
    if(ui->lineEdit_CAR_SetADD->text().count()!=4){
        ...
        val[0] = QString::number(QString::number(a[0]).toUShort()<<8).toUShort()|QString::number(a[1]).toUShort();
        val[1] = QString::number(QString::number(a[2]).toUShort()<<8).toUShort()|QString::number(a[3]).toUShort();
        if(ui->radioButton_CAR01_Calib->isChecked()){
...
void Module_MOD1103::on_pushButton_SampleFreq_clicked()
{
    ...
    m_edev_mod1103->writeSingleREG(EDev_Mod1103::dREG_CAR01_SPT,val&0xFFFF);
