#include "sensor_sen0101.h"
#include "ui_sensor_sen0101.h"


/////////////////////////////////////////////////////////////////////////////////////////////////Function for Sensor...

void Sensor_SEN0101::setDeviceWidget(VDevice dev,VUser user)
{
   ...
   //Create Sensor
   m_edev_sen0101 = new EDev_Sen0101();
   m_edev_sen0101->setSensorUnit(EDev_Sen0101::SSU_tempDegree);
   m_edev_sen0101->setSampleTime(m_modbusSampleTime);
   m_edev_sen0101->Start(dev);

    if(m_displayTimer)delete m_displayTimer;
    m_displayTimer = new QTimer();
    connect(m_displayTimer,&QTimer::timeout,this,&Sensor_SEN0101::on_displayTimer);
    m_displayTimer->start(m_displayInterval);
    ui->spinBox_SampleTime->setValue(m_modbusSampleTime);
}


void Sensor_SEN0101::sensorCalibration(double valueCalib,double valueRaw)
{
    if(m_edev_sen0101->isConnect()){
        ...
        case VDeviceCalibration::Calib_Sampling_OnePoint:
            m_edev_sen0101->setSampleTime(m_deviceCalibration->sampleTime());
            break;
        ...
        case VDeviceCalibration::Calib_WriteData:
            m_edev_sen0101->writeCalib(m_deviceCalibration->pointCalib(),m_deviceCalibration->valueCalib(m_edev_sen0101->resolution()));
...
m_deviceCalibration->setConnectStatus(m_edev_sen0101->isConnect());

}

void Sensor_SEN0101::setInfoCalib()
{
    if(m_user.accountType()==VUser::root){
        int _p = m_edev_sen0101->pointCalib();
        if(_p>0){
...
void Sensor_SEN0101::sensorWriteReport(bool status)
{
    switch(m_SWS_Status){
    case Sensor_SEN0101::SWS_SetZero:
        break;
    case Sensor_SEN0101::SWS_Calib:
        ...
        break;
    case Sensor_SEN0101::SWS_default:
        return;
    }
    m_SWS_Status = Sensor_SEN0101::SWS_default;
}

void Sensor_SEN0101::calculateParametes(double value, int resolution)
{
        //Calculate
...
void Sensor_SEN0101::on_displayTimer()
{
    if(m_edev_sen0101->isConnect()){
        m_displayTimer->setInterval(m_displayInterval);
        m_edev_sen0101->setSampleTime(m_modbusSampleTime);
        m_edev_sen0101->update();
        switch (m_edev_sen0101->pollStatus()) {
        ...
        case ESensors::sensorReadReply:
            this->displayPoll();
            m_edev_sen0101->setPollStatus(ESensors::sensorIdle);
            break;
        ...
        m_edev_sen0101->setSampleTime(500);
        if(m_displayToggle){
    ...
    //Chart Calib
    if(m_deviceCalibration)this->sensorCalibration(m_edev_sen0101->valueCalib(),m_edev_sen0101->value());
}
void Sensor_SEN0101::displayPoll()
{
    m_temp_Degree=m_edev_sen0101->valueCalib();
    ...
    ui->lcdNumber_Value->display(QString::number(m_temp_Degree,'f',m_edev_sen0101->resolution()));
    //calculate basic parameters
    this->calculateParametes(m_temp_Degree,m_edev_sen0101->resolution());
    //Chart graph
    if(m_vchartZoom){
...
void Sensor_SEN0101::on_setChartParameter(int sampleTime)
{
...
void Sensor_SEN0101::closeEvent(QCloseEvent *event)
{
    QMessageBox  msg;
    ...
    msg.exec();
     if (msg.result() != QMessageBox::Yes) {
         ...
         m_edev_sen0101->close();
         event->accept();
     }
}

Sensor_SEN0101::Sensor_SEN0101(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::Sensor_SEN0101)
{
    ui->setupUi(this);
...
void Sensor_SEN0101::on_pushButton_ChartZoom_clicked()
{
    if(m_vchartZoom) delete m_vchartZoom;
        ...
        connect(m_vchartZoom,&Vchart_Zoom::setChartParameter,this,&Sensor_SEN0101::on_setChartParameter);
        m_vchartZoom->show();
}

void Sensor_SEN0101::on_comboBox_Temp_Unit_currentIndexChanged(int index)
{
    switch(index){
    case 0:m_edev_sen0101->setSensorUnit(EDev_Sen0101::SSU_tempDegree);
        break;
    case 1:m_edev_sen0101->setSensorUnit(EDev_Sen0101::SSU_tempKelvin);
        break;
    case 2:m_edev_sen0101->setSensorUnit(EDev_Sen0101::SSU_tempFahrenheit);
        break;
    }
}

void Sensor_SEN0101::on_spinBox_SampleTime_valueChanged(int arg1)
{
    if(arg1>ui->spinBox_SampleTime->minimum())ui->pushButton_SetSampleTime->setEnabled(true);
}

void Sensor_SEN0101::on_pushButton_SetSampleTime_clicked()
{
...
void Sensor_SEN0101::on_pushButton_Calib_clicked()
{
    ...
    m_deviceCalibration->setResolution(m_edev_sen0101->resolution());
    ...
    m_deviceCalibration->setUnit(m_edev_sen0101->sensorUnitText());
    m_deviceCalibration->show();
}

void Sensor_SEN0101::on_pushButton_Exit_clicked()
{
    this->close();
}

void Sensor_SEN0101::on_pushButton_getSamples_clicked()
{
...
void Sensor_SEN0101::on_tabWidget_tabBarClicked(int index)
{
