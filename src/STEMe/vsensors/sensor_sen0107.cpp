#include "sensor_sen0107.h"
#include "ui_sensor_sen0107.h"

/////////////////////////////////////////////////////////////////////////////////////////////////Function for Sensor...


void Sensor_SEN0107::setDeviceWidget(VDevice dev,VUser user)
{
   ...
   //Create Sensor
   m_edev_sen0107 = new EDev_Sen0107();
   m_edev_sen0107->setSensorUnit(EDev_Sen0107::SSU_mg);
   m_edev_sen0107->setSampleTime(m_modbusSampleTime);
   m_edev_sen0107->Start(dev);

   if(m_displayTimer)delete m_displayTimer;
   m_displayTimer = new QTimer();
   connect(m_displayTimer,&QTimer::timeout,this,&Sensor_SEN0107::on_displayTimer);
...
void Sensor_SEN0107::sensorCalibration(double valueCalib,double valueRaw)
{
    m_GET_SAMPLE_SUM+=m_edev_sen0107->REG_sen01_VALUE();
        m_GET_SAMPLE_COUNT++;
        m_deviceCalibration->setValueCalib_Y1(m_edev_sen0107->temperatureTable());
        if(m_edev_sen0107->isConnect()){
            ...
            case VDeviceCalibration::Calib_Sampling_OnePoint:
                m_edev_sen0107->setSampleTime(m_deviceCalibration->sampleTime());
                break;
            ...
            case VDeviceCalibration::Calib_WriteData:
                m_deviceCalibration->setValueCalib(m_deviceCalibration->pointCalib(),QVariant(m_GET_SAMPLE_SUM/QVariant(m_GET_SAMPLE_COUNT).toDouble()).toDouble()/1000.0,m_edev_sen0107->temperature());
                m_edev_sen0107->writeCalib(m_deviceCalibration->pointCalib(),m_deviceCalibration->valueCalib(m_edev_sen0107->resolution()));
    ...
    m_deviceCalibration->setConnectStatus(m_edev_sen0107->isConnect());

}

void Sensor_SEN0107::setInfoCalib()
{
    if(m_user.accountType()==VUser::root){
        int _p = m_edev_sen0107->pointCalib();
        if(_p>0){
...
void Sensor_SEN0107::sensorWriteReport(bool status)
{
    switch(m_SWS_Status){
    case Sensor_SEN0107::SWS_SetZero:
        if(!status){
    ...
    case Sensor_SEN0107::SWS_Calib:
        ...
        break;
    case Sensor_SEN0107::SWS_default:
        return;
    }
    m_SWS_Status = Sensor_SEN0107::SWS_default;
}

void Sensor_SEN0107::calculateParametes(double value, int resolution)
{
        //Calculate
...
void Sensor_SEN0107::on_displayTimer()
{
    if(m_edev_sen0107->isConnect()){
        m_displayTimer->setInterval(m_displayInterval);
        m_edev_sen0107->setSampleTime(m_modbusSampleTime);
        m_edev_sen0107->update();
        switch (m_edev_sen0107->pollStatus()) {
        ...
        case ESensors::sensorReadReply:
            this->displayPoll();
            m_edev_sen0107->setPollStatus(ESensors::sensorIdle);
            break;
        ...
        m_edev_sen0107->setSampleTime(500);
        if(m_displayToggle){
    ...
    //Chart Calib
    if(m_deviceCalibration)this->sensorCalibration(m_edev_sen0107->valueCalib(),m_edev_sen0107->value());
}
void Sensor_SEN0107::displayPoll()
{
    double _Salt = m_edev_sen0107->valueCalib();
    double _temp=25;
    if(ui->checkBox_setTemperature->isChecked())_temp = VDevice::decryption(m_edev_sen0107->REG_temp_VALUE());
    if(_temp>=0xFFFFF){
        _temp=25;
    ...
    m_edev_sen0107->setTemperature(_temp);
    ui->lcdNumber_Temp->display(_temp);
    //get average of value
    if(ui->checkBox_setMathAvg->isChecked()){
        _Salt = VDevice::mathAvg(_Salt,&m_VALUE_AVG,ui->spinBox_setSample->value());
        }
    ui->lcdNumber_Value->display(QString::number(_Salt,'f',m_edev_sen0107->resolution()));
    //calculate basic parameters
    this->calculateParametes(_Salt,m_edev_sen0107->resolution());
    //Chart graph
    if(m_vchartZoom){
        m_vchartZoom->vchartGraph(_Salt,ui->comboBox_Unit->currentText());
...
void Sensor_SEN0107::on_setChartParameter(int sampleTime)
{
...
Sensor_SEN0107::Sensor_SEN0107(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::Sensor_SEN0107)
{
    ui->setupUi(this);


}
Sensor_SEN0107::~Sensor_SEN0107()
{
    delete ui;
}

void Sensor_SEN0107::closeEvent(QCloseEvent *event)
{
    QMessageBox  msg;
    ...
    msg.exec();
     if (msg.result() != QMessageBox::Yes) {
         ...
         m_edev_sen0107->close();
         event->accept();
     }
}

void Sensor_SEN0107::on_pushButton_Exit_clicked()
{
    this->close();
}

void Sensor_SEN0107::on_pushButton_ChartZoom_clicked()
{
    if(m_vchartZoom) delete m_vchartZoom;
        ...
        connect(m_vchartZoom,&Vchart_Zoom::setChartParameter,this,&Sensor_SEN0107::on_setChartParameter);
        m_vchartZoom->show();
}

void Sensor_SEN0107::on_pushButton_getSamples_clicked()
{
...
void Sensor_SEN0107::on_spinBox_SampleTime_valueChanged(int arg1)
{
    if(arg1>ui->spinBox_SampleTime->minimum())ui->pushButton_SetSampleTime->setEnabled(true);
}

void Sensor_SEN0107::on_pushButton_SetSampleTime_clicked()
{
...
void Sensor_SEN0107::on_pushButton_Calib_clicked()
{
    ...
    m_deviceCalibration->setResolution(m_edev_sen0107->resolution());
    m_deviceCalibration->setTolerance(3);
    m_deviceCalibration->setCalibTime(5);
    m_deviceCalibration->setSampleNumber(50);
    m_deviceCalibration->setValueCalib_Y1(8.25);
    m_deviceCalibration->setUnit(m_edev_sen0107->sensorUnitText());
    m_deviceCalibration->show();
}

void Sensor_SEN0107::on_tabWidget_tabBarClicked(int index)
{
    if(index==2)this->setInfoCalib();
}

void Sensor_SEN0107::on_comboBox_Unit_currentIndexChanged(int index)
{
    switch(index){
    case 0:m_edev_sen0107->setSensorUnit(EDev_Sen0107::SSU_mg);
        break;
    }
}

void Sensor_SEN0107::on_pushButton_SetZero_clicked()
{
    ui->pushButton_SetZero->setEnabled(false);
    QVector<quint16> values(2);
    values[0] = (m_edev_sen0107->REG_sen01_VALUE()>>16)&0xFFFF;
    values[1] = (m_edev_sen0107->REG_sen01_VALUE()&0xFFFF);
    m_SWS_Status = SWS_SetZero;
    m_edev_sen0107->writeZero(values);
    ui->pushButton_SetZero->setEnabled(true);
}
