#include "sensor_sen0109.h"
#include "ui_sensor_sen0109.h"

/////////////////////////////////////////////////////////////////////////////////////////////////Function for Sensor...

void Sensor_SEN0109::setDeviceWidget(VDevice dev,VUser user)
{
   ...
   //Create Sensor
   m_edev_sen0109 = new EDev_Sen0109();
   m_edev_sen0109->setSensorUnit(EDev_Sen0109::SSU_ppt);
   m_edev_sen0109->setSampleTime(m_modbusSampleTime);
   m_edev_sen0109->Start(dev);

   if(m_displayTimer)delete m_displayTimer;
   m_displayTimer = new QTimer();
   connect(m_displayTimer,&QTimer::timeout,this,&Sensor_SEN0109::on_displayTimer);
...
void Sensor_SEN0109::sensorCalibration(double valueCalib,double valueRaw)
{
    if(m_edev_sen0109->isConnect()){
        ...
        case VDeviceCalibration::Calib_Sampling_OnePoint:
            m_edev_sen0109->setSampleTime(m_deviceCalibration->sampleTime());
            break;
        ...
        case VDeviceCalibration::Calib_WriteData:
            m_edev_sen0109->writeCalib(m_deviceCalibration->pointCalib(),m_deviceCalibration->valueCalib(m_edev_sen0109->resolution()));
...
m_deviceCalibration->setConnectStatus(m_edev_sen0109->isConnect());

}

void Sensor_SEN0109::setInfoCalib()
{
    if(m_user.accountType()==VUser::root){
        int _p = m_edev_sen0109->pointCalib();
        if(_p>0){
...
void Sensor_SEN0109::sensorWriteReport(bool status)
{
    switch(m_SWS_Status){
    case Sensor_SEN0109::SWS_SetZero:
        if(!status){
    ...
    case Sensor_SEN0109::SWS_Calib:
        ...
        break;
    case Sensor_SEN0109::SWS_default:
        return;
    }
    m_SWS_Status = Sensor_SEN0109::SWS_default;
}

void Sensor_SEN0109::calculateParametes(double value, int resolution)
{
        //Calculate
...
void Sensor_SEN0109::on_displayTimer()
{
    if(m_edev_sen0109->isConnect()){
        m_displayTimer->setInterval(m_displayInterval);
        m_edev_sen0109->setSampleTime(m_modbusSampleTime);
        m_edev_sen0109->update();
        switch (m_edev_sen0109->pollStatus()) {
        ...
        case ESensors::sensorReadReply:
            this->displayPoll();
            m_edev_sen0109->setPollStatus(ESensors::sensorIdle);
            break;
        ...
        m_edev_sen0109->setSampleTime(500);
        if(m_displayToggle){
    ...
    //Chart Calib
    if(m_deviceCalibration)this->sensorCalibration(m_edev_sen0109->valueCalib(),m_edev_sen0109->value());
}
void Sensor_SEN0109::displayPoll()
{
    double _Salt = m_edev_sen0109->valueCalib();
    double _temp=25;
    if(ui->checkBox_setTemperature->isChecked())_temp = m_edev_sen0109->temperature();
        else m_edev_sen0109->setTemperature(_temp);
    if(_temp>=0xFFFFF){
        _temp=0;
    ...
    ui->lcdNumber_Value->display(QString::number(_Salt,'f',m_edev_sen0109->resolution()));
    //calculate basic parameters
    this->calculateParametes(_Salt,m_edev_sen0109->resolution());
    //Chart graph
    if(m_vchartZoom){
...
void Sensor_SEN0109::on_setChartParameter(int sampleTime)
{
...
Sensor_SEN0109::Sensor_SEN0109(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::Sensor_SEN0109)
{
    ui->setupUi(this);


}
Sensor_SEN0109::~Sensor_SEN0109()
{
    delete ui;
}

void Sensor_SEN0109::closeEvent(QCloseEvent *event)
{
    QMessageBox  msg;
    ...
    msg.exec();
     if (msg.result() != QMessageBox::Yes) {
         ...
         m_edev_sen0109->close();
         event->accept();
     }
}

void Sensor_SEN0109::on_pushButton_Exit_clicked()
{
    this->close();
}

void Sensor_SEN0109::on_pushButton_ChartZoom_clicked()
{
    if(m_vchartZoom) delete m_vchartZoom;
        ...
        connect(m_vchartZoom,&Vchart_Zoom::setChartParameter,this,&Sensor_SEN0109::on_setChartParameter);
        m_vchartZoom->show();
}

void Sensor_SEN0109::on_pushButton_getSamples_clicked()
{
...
void Sensor_SEN0109::on_spinBox_SampleTime_valueChanged(int arg1)
{
    if(arg1>ui->spinBox_SampleTime->minimum())ui->pushButton_SetSampleTime->setEnabled(true);
}

void Sensor_SEN0109::on_pushButton_SetSampleTime_clicked()
{
...
void Sensor_SEN0109::on_pushButton_Calib_clicked()
{
    ...
    m_deviceCalibration->setResolution(m_edev_sen0109->resolution());
    ...
    m_deviceCalibration->setUnit(m_edev_sen0109->sensorUnitText());
    m_deviceCalibration->show();
}

void Sensor_SEN0109::on_tabWidget_tabBarClicked(int index)
{
    if(index==2)this->setInfoCalib();
}

void Sensor_SEN0109::on_comboBox_Unit_currentIndexChanged(int index)
{
    switch(index){
    case 0:m_edev_sen0109->setSensorUnit(EDev_Sen0109::SSU_ppt);
        break;
    case 1:m_edev_sen0109->setSensorUnit(EDev_Sen0109::SSU_ppm);
        break;
    }
}

void Sensor_SEN0109::on_pushButton_SetZero_clicked()
{
    ui->pushButton_SetZero->setEnabled(false);
    QVector<quint16> values(2);
    values[0] = (m_edev_sen0109->REG_sen01_VALUE()>>16)&0xFFFF;
    values[1] = (m_edev_sen0109->REG_sen01_VALUE()&0xFFFF);
    m_SWS_Status = SWS_SetZero;
    m_edev_sen0109->writeZero(values);
    ui->pushButton_SetZero->setEnabled(true);
}
