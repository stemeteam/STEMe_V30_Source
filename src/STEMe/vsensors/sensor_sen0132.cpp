#include "sensor_sen0132.h"
#include "ui_sensor_sen0132.h"
...
void Sensor_SEN0132::setDeviceWidget(VDevice dev,VUser user)
{
   ...
   //Create Sensor
   m_edev_sen0132 = new EDev_Sen0132();
   m_edev_sen0132->setSensorUnit(EDev_Sen0132::SSU_humidityRelative);
   m_edev_sen0132->setSampleTime(m_modbusSampleTime);
   m_edev_sen0132->Start(dev);

   if(m_displayTimer)delete m_displayTimer;
   m_displayTimer = new QTimer();
   connect(m_displayTimer,&QTimer::timeout,this,&Sensor_SEN0132::on_displayTimer);
...
void Sensor_SEN0132::sensorCalibration(double valueCalib,double valueRaw)
{
    if(m_edev_sen0132->isConnect()){
        ...
        case VDeviceCalibration::Calib_Sampling_OnePoint:
            m_edev_sen0132->setSampleTime(m_deviceCalibration->sampleTime());
            break;
        ...
        case VDeviceCalibration::Calib_WriteData:
            m_edev_sen0132->writeCalib(m_deviceCalibration->pointCalib(),m_deviceCalibration->valueCalib(m_edev_sen0132->resolution()));
...
m_deviceCalibration->setConnectStatus(m_edev_sen0132->isConnect());

}

void Sensor_SEN0132::setInfoCalib()
{
    if(m_user.accountType()==VUser::root){
        int _p = m_edev_sen0132->pointCalib();
        if(_p>0){
...
void Sensor_SEN0132::sensorWriteReport(bool status)
{
    switch(m_SWS_Status){
    case Sensor_SEN0132::SWS_SetZero:
        if(!status){
        ...
        m_SWS_Status = Sensor_SEN0132::SWS_default;
        break;
    case Sensor_SEN0132::SWS_Calib:
        ...
        break;
    case Sensor_SEN0132::SWS_default:
        return;
    }
    m_SWS_Status = Sensor_SEN0132::SWS_default;
}

void Sensor_SEN0132::calculateParametes(double value, int resolution)
{
        //Calculate
...
void Sensor_SEN0132::on_displayTimer()
{
    if(m_edev_sen0132->isConnect()){
        m_displayTimer->setInterval(m_displayInterval);
        m_edev_sen0132->setSampleTime(m_modbusSampleTime);
        m_edev_sen0132->update();
        switch (m_edev_sen0132->pollStatus()) {
        ...
        case ESensors::sensorReadReply:
            this->displayPoll();
            m_edev_sen0132->setPollStatus(ESensors::sensorIdle);
            break;
        ...
        m_edev_sen0132->setSampleTime(500);
        if(m_displayToggle){
    ...
    //Chart Calib
    if(m_deviceCalibration)this->sensorCalibration(m_edev_sen0132->valueCalib(),m_edev_sen0132->value());
}
void Sensor_SEN0132::displayPoll()
{
    double _humidity = m_edev_sen0132->valueCalib();
    ...
    ui->lcdNumber_Value->display(QString::number(_humidity,'f',m_edev_sen0132->resolution()));
    //calculate basic parameters
    this->calculateParametes(_humidity,m_edev_sen0132->resolution());
    //Chart graph
    if(m_vchartZoom){
...
void Sensor_SEN0132::on_setChartParameter(int sampleTime)
{
...
Sensor_SEN0132::Sensor_SEN0132(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::Sensor_SEN0132)
{
    ui->setupUi(this);


}
Sensor_SEN0132::~Sensor_SEN0132()
{
    delete ui;
}

void Sensor_SEN0132::closeEvent(QCloseEvent *event)
{
    QMessageBox  msg;
    ...
    msg.exec();
     if (msg.result() != QMessageBox::Yes) {
         ...
         m_edev_sen0132->close();
         event->accept();
     }
}

void Sensor_SEN0132::on_pushButton_Exit_clicked()
{
    this->close();
}

void Sensor_SEN0132::on_pushButton_ChartZoom_clicked()
{
    if(m_vchartZoom) delete m_vchartZoom;
        ...
        connect(m_vchartZoom,&Vchart_Zoom::setChartParameter,this,&Sensor_SEN0132::on_setChartParameter);
        m_vchartZoom->show();
}

void Sensor_SEN0132::on_pushButton_getSamples_clicked()
{
...
void Sensor_SEN0132::on_comboBox_Temp_Unit_currentIndexChanged(int index)
{
    switch(index){
    case 0:m_edev_sen0132->setSensorUnit(EDev_Sen0132::SSU_humidityRelative);
        break;
    }
}

void Sensor_SEN0132::on_spinBox_SampleTime_valueChanged(int arg1)
{
    if(arg1>ui->spinBox_SampleTime->minimum())ui->pushButton_SetSampleTime->setEnabled(true);
}

void Sensor_SEN0132::on_pushButton_SetSampleTime_clicked()
{
...
void Sensor_SEN0132::on_pushButton_Calib_clicked()
{
    ...
    m_deviceCalibration->setResolution(m_edev_sen0132->resolution());
    ...
    m_deviceCalibration->setUnit(m_edev_sen0132->sensorUnitText());
    m_deviceCalibration->show();
}

void Sensor_SEN0132::on_tabWidget_tabBarClicked(int index)
{
    if(index==2)this->setInfoCalib();
}

void Sensor_SEN0132::on_pushButton_SetZero_clicked()
{
    ui->pushButton_SetZero->setEnabled(false);
    QVector<quint16> values(2);
    values[0] = (m_edev_sen0132->REG_sen01_ADC()>>16)&0xFFFF;
    values[1] = (m_edev_sen0132->REG_sen01_ADC()&0xFFFF);
    m_SWS_Status = SWS_SetZero;
    m_edev_sen0132->setZero(values);
}
