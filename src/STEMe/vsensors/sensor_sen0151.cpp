#include "sensor_sen0151.h"
#include "ui_sensor_sen0151.h"


/////////////////////////////////////////////////////////////////////////////////////////////////Function for Sensor...

void Sensor_SEN0151::setDeviceWidget(VDevice dev,VUser user)
{
   ...
   //Create Sensor
   m_edev_sen0151 = new EDev_Sen0151();
   m_edev_sen0151->setSensorUnit(EDev_Sen0151::SSU_amp);
   m_edev_sen0151->setSampleTime(m_modbusSampleTime);
   m_edev_sen0151->Start(dev);

    if(m_displayTimer)delete m_displayTimer;
    m_displayTimer = new QTimer();
    connect(m_displayTimer,&QTimer::timeout,this,&Sensor_SEN0151::on_displayTimer);
    m_displayTimer->start(m_displayInterval);
    ui->spinBox_SampleTime->setValue(m_modbusSampleTime);
}


void Sensor_SEN0151::sensorCalibration(double valueCalib,double valueRaw)
{
    if(m_edev_sen0151->isConnect()){
        ...
        case VDeviceCalibration::Calib_Sampling_OnePoint:
            m_edev_sen0151->setSampleTime(m_deviceCalibration->sampleTime());
            break;
        ...
        case VDeviceCalibration::Calib_WriteData:
            m_edev_sen0151->writeCalib(m_deviceCalibration->pointCalib(),m_deviceCalibration->valueCalib(m_edev_sen0151->resolution()));
...
m_deviceCalibration->setConnectStatus(m_edev_sen0151->isConnect());

}

void Sensor_SEN0151::setInfoCalib()
{
    if(m_user.accountType()==VUser::root){
        int _p = m_edev_sen0151->pointCalib();
        if(_p>0){
...
void Sensor_SEN0151::sensorWriteReport(bool status)
{
    switch(m_SWS_Status){
    case Sensor_SEN0151::SWS_SetZero:
        if(!status){
    ...
    case Sensor_SEN0151::SWS_Calib:
        ...
        break;
    case Sensor_SEN0151::SWS_default:
        return;
    }
    m_SWS_Status = Sensor_SEN0151::SWS_default;
}

void Sensor_SEN0151::calculateParametes(double value, int resolution)
{
        //Calculate
...
void Sensor_SEN0151::on_displayTimer()
{
    if(m_edev_sen0151->isConnect()){
        m_displayTimer->setInterval(m_displayInterval);
        m_edev_sen0151->setSampleTime(m_modbusSampleTime);
        m_edev_sen0151->update();
        switch (m_edev_sen0151->pollStatus()) {
        ...
        case ESensors::sensorReadReply:
            this->displayPoll();
            m_edev_sen0151->setPollStatus(ESensors::sensorIdle);
            break;
        ...
        m_edev_sen0151->setSampleTime(500);
        if(m_displayToggle){
    ...
    //Chart Calib
    if(m_deviceCalibration)this->sensorCalibration(m_edev_sen0151->valueCalib(),m_edev_sen0151->value());
}
void Sensor_SEN0151::displayPoll()
{
    m_current_ampe=m_edev_sen0151->valueCalib();
    //get average of value
    if(ui->checkBox_setMathAvg->isChecked())m_current_ampe = VDevice::mathAvg(m_current_ampe,&m_VALUE_AVG,ui->spinBox_setSample->value());
    ui->lcdNumber_Value->display(QString::number(m_current_ampe,'f',m_edev_sen0151->resolution()));
    //calculate basic parameters
    this->calculateParametes(m_current_ampe,m_edev_sen0151->resolution());
    //Chart graph
    if(m_vchartZoom){
        m_vchartZoom->vchartGraph(m_current_ampe,ui->comboBox_Temp_Unit->currentText());
...
void Sensor_SEN0151::on_setChartParameter(int sampleTime)
{
    m_modbusSampleTime = QString::number(sampleTime).toUShort();
    ui->spinBox_SampleTime->setValue(sampleTime);
    m_edev_sen0151->setSampleTime(m_modbusSampleTime);
   // m_vsensor->setSampleTime(m_modbusSampleTime);
}

void Sensor_SEN0151::closeEvent(QCloseEvent *event)
{
    QMessageBox  msg;
    ...
    msg.exec();
     if (msg.result() != QMessageBox::Yes) {
         ...
         m_edev_sen0151->close();
         event->accept();
     }
}

Sensor_SEN0151::Sensor_SEN0151(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::Sensor_SEN0151)
{
    ui->setupUi(this);
...
void Sensor_SEN0151::on_pushButton_ChartZoom_clicked()
{
    if(m_vchartZoom) delete m_vchartZoom;
        ...
        connect(m_vchartZoom,&Vchart_Zoom::setChartParameter,this,&Sensor_SEN0151::on_setChartParameter);
        m_vchartZoom->show();
}

void Sensor_SEN0151::on_comboBox_Temp_Unit_currentIndexChanged(int index)
{
    switch(index){
    case 0:m_edev_sen0151->setSensorUnit(EDev_Sen0151::SSU_amp);
        break;
    case 1:m_edev_sen0151->setSensorUnit(EDev_Sen0151::SSU_milliAmp);
        break;
    }
}

void Sensor_SEN0151::on_spinBox_SampleTime_valueChanged(int arg1)
{
    if(arg1>ui->spinBox_SampleTime->minimum())ui->pushButton_SetSampleTime->setEnabled(true);
}

void Sensor_SEN0151::on_pushButton_SetSampleTime_clicked()
{
...
void Sensor_SEN0151::on_pushButton_Calib_clicked()
{
    ...
    m_deviceCalibration->setResolution(m_edev_sen0151->resolution());
    m_deviceCalibration->setTolerance(1);
    m_deviceCalibration->setCalibTime(30);
    m_deviceCalibration->setSampleNumber(300);
    m_deviceCalibration->setValueCalib_Y1(3);
    m_deviceCalibration->setUnit(m_edev_sen0151->sensorUnitText());
    m_deviceCalibration->show();
}

void Sensor_SEN0151::on_pushButton_Exit_clicked()
{
    this->close();
}

void Sensor_SEN0151::on_pushButton_getSamples_clicked()
{
...
void Sensor_SEN0151::on_tabWidget_tabBarClicked(int index)
{
    if(index==2)this->setInfoCalib();
}

void Sensor_SEN0151::on_pushButton_SetZero_clicked()
{
    ui->pushButton_SetZero->setEnabled(false);
    QVector<quint16> values(2);
    values[0] = (m_edev_sen0151->REG_sen01_ADC()>>16)&0xFFFF;
    values[1] = (m_edev_sen0151->REG_sen01_ADC()&0xFFFF);
    m_SWS_Status = SWS_SetZero;
    m_edev_sen0151->writeZero(values);
    ui->pushButton_SetZero->setEnabled(true);
}
